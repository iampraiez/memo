...
import { ChartLine, Database, Users, ArrowsClockwise, Trash, Shield } from "@phosphor-icons/react";
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold text-neutral-900">Admin Center</h1>
          <p className="text-neutral-600 mt-1">System intelligence and node management</p>
        </div>
          ...
          {["overview", "jobs", "logs"].map((id) => (
            <button
              ...
              className={`py-4 border-b-2 font-bold text-sm uppercase tracking-widest transition-all ${
                activeTab === id ? "border-primary-900 text-primary-900" : "border-transparent text-neutral-400 hover:text-neutral-600"
              }`}
            >
              {id}
            </button>
      ...
      {activeTab === "overview" && (
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <StatCard title="Active Users" value={stats.activeUsers.toLocaleString()} icon={ChartLine} />
            <StatCard title="Total Memories" value={stats.totalMemories.toLocaleString()} icon={Database} />
            <StatCard title="Storage Used" value={stats.storageUsed} icon={Database} />
          </div>
          
          <Card className="p-8 space-y-6">
            <h2 className="text-xl font-display font-bold text-neutral-900">System Health</h2>
